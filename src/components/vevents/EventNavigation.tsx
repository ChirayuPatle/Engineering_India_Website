...
const EventNavigation = ({ className }: EventNavigationProps) => {
  ...
  return (
    <nav className={cn("sticky top-4 z-10", className)}>
      <div className="hide-scrollbar flex items-center justify-between overflow-x-auto rounded-lg bg-white p-1.5 shadow-md">
        {navItems.map((item) => (
          <button
            key={item.id}
            onClick={() => scrollToSection(item.id)}
            className={cn(
              "flex items-center gap-1.5 whitespace-nowrap rounded-md px-3 py-2 text-sm transition-all",
